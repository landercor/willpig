...
export async function verBiblioteca(req, res) {
  ...
  res.render('biblioteca', { tituloPagina: 'Biblioteca | Willpig Studio', historias: error ? [] : data || [], loggerUser: req.session?.user || null });
}
...
export async function verBusqueda(req, res) {
  ...
  const [{ data: resultados }, { data: usuarios }] = await Promise.all([
    q ? publicQuery().ilike('titulo', '%' + q + '%').order('created_at', { ascending: false }) : Promise.resolve({ data: [] }),
    q ? db.from('cuenta_usuario').select('id_cuenta_usuario, username, avatar_url, biografia').ilike('username', '%' + q + '%').limit(12) : Promise.resolve({ data: [] }),
  ]);
...
export async function verCategoria(req, res) {
  ...
  res.render('categoria', { tituloPagina: matches[0].nombre + ' | Willpig Studio', categoriaNombre: matches[0].nombre, historias: historias || [], loggerUser: req.session?.user || null });
}
