...
export async function getDashboard(req, res) {
  ...
  await renderAdmin(req, res, 'dashboard', {
...
export async function getCapitulos(req, res) { const { data } = await db.from('capitulos').select('*, cuentos(titulo)').order('created_at', { ascending: false }); await renderAdmin(req, res, 'capitulos', { capitulos: data || [] }); }
...
export async function getComentarios(req, res) { const { data } = await db.from('comentarios').select('*, cuenta_usuario(username), cuentos(titulo)').order('created_at', { ascending: false }); await renderAdmin(req, res, 'comentarios', { comentarios: data || [] }); }
...
export const getCatalogo = name => async (req, res) => {
  ...
  if (name === 'notificaciones') return await renderAdmin(req, res, name, { notificaciones: [] });
  const query = db.from(name).select(name === 'notificaciones' ? '*, cuenta_usuario(username)' : '*').order(tablePk[name]); const { data } = await query; const rows = name === 'notificaciones' ? (data || []).map(n => ({ ...n, mensaje: n.contenido, leida: n.vista, username: n.cuenta_usuario?.username || n.cuenta_usuario_id })) : data || []; await renderAdmin(req, res, name, { [name]: rows });
};
