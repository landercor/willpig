...
export async function markUserNotificationsRead(req, res) { const { error } = await db.from('notificaciones').update({ vista: true }).eq('cuenta_usuario_id', userId(req)); if (error) console.error("Error marking notif:", error); res.json({ ok: true }); }
